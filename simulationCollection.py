...
#######################################################################################################################
###################################### Heat flux graph cylinder description creation ##################################
#######################################################################################################################
...
#######################################################################################################################
###################################### Heat flux graph plates description creation ####################################
#######################################################################################################################


def heat_flux_graph_plates_inital_values(e1=0.4, T1=275, T2=280, emissivity_step_nb=500):
    points = []
    x_axis = []
    sigma = 5.6704 * pow(10, -8)
    for epsilon2 in range(emissivity_step_nb):
        e2 = (epsilon2 + 1) / emissivity_step_nb
        x_axis.append(e2)
        points.append((sigma * (pow(T1, 4) - pow(T2, 4))) / (1 / e1 + 1 / e2 - 1))
    return x_axis, points


heat_flux_graph_plates_variables = [VariableDescription("temperature1", "Upper temperature [K]", 275, 350, 0.1, 275),
                                    VariableDescription("temperature2", "Lower temperature [K]", 275, 350, 0.1, 280),
                                    VariableDescription("emissivity1", "Upper ε", 0, 1, 0.002, 0.4)]

heat_flux_graph_plates_formula = """
        var data = source.data;
        var e1 = emissivity1.value;
        var T1 = temperature1.value;
        var T2 = temperature2.value;
...
heat_flux_graph_plates_description = GraphDescription(heat_flux_graph_plates_variables, heat_flux_graph_plates_formula,
                                                      heat_flux_graph_plates_inital_values(), [0, 1], [-10, 0],
                                                      "Heat flux value for two infinite plates", 400, 600, "Lower ε",
                                                      "Heat flux value []")
